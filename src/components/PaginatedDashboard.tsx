...
export default function PaginatedDashboard({ sessionId, analystEmail, onLogout }: DashboardProps) {
  ...
  const handleGetMyBatch = async () => {
    try {
      ...
      const batch = await candidateService.getNextUnassignedBatch(analystEmail, 20);
      ...
      setFilters({ ...filters, assignedTo: analystEmail, status: 'pending' });
      setPage(1);
  ...
  const mapCandidateToLegacyFormat = (c: Candidate) => ({
    ...
    admCurriculo: c.adm_curriculo,
    admDiploma: c.adm_diploma,
    admDocumentos: c.adm_documentos,
    admCursos: c.adm_cursos,
    assistCurriculo: c.assist_curriculo,
    assistDiploma: c.assist_diploma,
    assistCarteira: c.assist_carteira,
    assistCursos: c.assist_cursos,
    assistDocumentos: c.assist_documentos,
    statusTriagem: c.status_triagem,
    dataHoraTriagem: c.data_hora_triagem,
    analistaTriagem: c.analista_triagem
  });
      ...
      {loading && candidates.length === 0 ? (
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="flex-1 overflow-hidden">
            {selectedCandidate ? (
              <DocumentViewer
                candidate={mapCandidateToLegacyFormat(selectedCandidate)}
                onFocusDocument={() => {}}
              />
            ) : (
